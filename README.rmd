...
The plot below shows these three data sets. Note the strong undersampling in b) as shown by the small bar on the left. In c), these values have been trimmed off and there is not white space below x = `r round(xmin_est, 1)`. Note that the trimming can be quite large despite the relatively small `xmin_est` value. In this case, it removed \~5% of the data, but it can sometimes remove much more (such as 25 or 50%). That is usually not a problem as long as the remaining data has enough individuals to fit reliably, usually 300 or so.

```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}

set.seed(20202)
data_to_plot = bind_rows(unbiased_data %>% mutate(data = "a) Unbiased data\n(no undersampling)"), 
                         biased_data %>% mutate(data = "b) Biased data\n(small sizes undersampled)"),
                         mutate(biased_data_fixed %>% mutate(data = "c) Trimmed data\n(undersampled sizes removed)")))

sample_sizes = data_to_plot %>% group_by(data) %>% 
  tally() %>% 
  mutate(n = paste("n =", n))

data_to_plot %>%
  ggplot(aes(x = x, fill = data)) + 
  ...
  geom_text(data = sample_sizes, aes(label = n),
            x = 500, y = 100)
